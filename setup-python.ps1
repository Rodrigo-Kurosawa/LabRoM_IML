...
# Localizar Python 3.9+ (evita stub do Microsoft Store)
$pythonCmd = $null
...
if (-not $pythonCmd) {
    foreach ($c in @("python", "python3", "py")) {
        ...
        if ($found -and $found.Source -notlike "*WindowsApps*") {
            ...
            if ($ver -match '^\d+$' -and [int]$ver -ge 39) {
...
if (-not $pythonCmd) {
    Write-Host "ERRO: Python 3.9+ nao encontrado."
    Write-Host ""
    Write-Host "Instale Python de: https://www.python.org/downloads/"
    Write-Host "  - Marque a opcao 'Add python.exe to PATH' durante a instalacao."
    Write-Host "  - Reinicie o PowerShell apos instalar e execute este script novamente."
    exit 1
}
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "ERRO: Falha ao atualizar pip."
    exit 1
}

& "$VENV_DIR\Scripts\pip.exe" install ultralytics torch torchvision opencv-python pydicom pillow grad-cam "pylibjpeg[all]" --quiet
if ($LASTEXITCODE -ne 0) {
