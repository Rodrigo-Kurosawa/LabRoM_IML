...
# Detect if the file is truly XZ-compressed or already a plain JAR (local build)
if file weasis-core-img.jar.xz 2>/dev/null | grep -q "XZ compressed"; then
  xz --decompress weasis-core-img.jar.xz
else
  # Local build: file is already a plain JAR, just rename it
  mv weasis-core-img.jar.xz weasis-core-img.jar
fi
ARC_OS=$("$JDK_PATH_UNIX/bin/java" -cp "weasis-core-img.jar" org.weasis.opencv.natives.NativeLibrary 2>/dev/null || \
         "$JDK_PATH_UNIX/bin/java" -cp "weasis-core-img.jar" org.weasis.core.util.NativeLibrary 2>/dev/null || true)
